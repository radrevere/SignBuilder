...
class DividerRow extends React.Component{
    ...
    render(){
        ...
        return (<tr key={"divider" + Number(this.state.line)}><td>
            &nbsp;&nbsp;&nbsp;&nbsp;<b>Divider {(this.state.line+1)}:</b></td><td> Type<br/>
            <select onChange={(e) => this.onChangeDivider(e,this.state.line)} value={this.data.type}>
            {optionLists.dividerType.map(itm => (<option key={"divider" + String(itm.id)} value={itm.id}>{itm.name}</option>))}
            </select></td></tr>);
    }
}
...
class TextRow extends React.Component{
    ...
    render(){
        return (<tr key={"Line" + this.state.line}><td >&nbsp;&nbsp;&nbsp;&nbsp;<b>Line {(this.state.line+1)}:</b></td>
            <td>Text<br/><input value={this.data.text} onChange={(e)=>this.onChangeText(e,this.state.line)} placeholder="Sign Text"/></td>
            <td>Center <br/><input type="checkbox" onChange={(e)=>this.onClickCenter(e,this.state.line)} checked={this.state.center}/></td>
            <td>Editable <br/><input type="checkbox" onChange={(e)=>this.onClickCanEdit(e,this.state.line)} checked={this.state.canEdit}/></td>
            </tr>);
    }
}
...
class ShapeRow extends React.Component{
    ...
    render(){
        return (<tr key={"Shape"+(this.state.line)}><td>
            &nbsp;&nbsp;&nbsp;&nbsp;<b>Shape {this.state.line+1}:</b> </td><td>Shape<br/><select onChange={(e)=>this.onChangeShape(e,this.state.line)} value={this.data.shape}>{optionLists.shapeObject.map(itm => (<option key={"divider" + String(itm.id)} value={itm.id}>{itm.name}</option>))}</select>
            </td><td>Color <br/><select onChange={(e)=>this.onChangeColor(e,this.state.line)} value={this.data.color}>{optionLists.signColors.map(itm => (<option key={this.state.line + String(itm.id)} value={itm.id}>{itm.name}</option>))}</select>
            </td></tr>);
    }
}
...
class SymbolRow extends React.Component{
    ...
    {
        return(
            <React.Fragment key={"Symbol"+this.state.line}>
            <tr>
                <td>&nbsp;&nbsp;&nbsp;&nbsp;<b>Symbol {this.state.line+1}: </b></td><td>Category <br/><select onChange={(e)=>this.onChangeCategory(e,this.state.line)} value={this.state.catIdx}>{optionLists.symbolCategory.map(itm => (<option key={this.state.line + String(itm.id)} value={itm.id}>{itm.name}</option>))}</select>
                </td><td>&nbsp;Symbol <br/><select onChange={(e)=>this.onChangeSymbol(e,this.state.line)} value={this.state.symIdx}>{optionLists.symbols[this.state.catIdx].items.map(itm => (<option key={this.state.line + String(itm.id)} value={itm.value}>{itm.name}</option>))}</select>
                </td><td><svg width="72px" height="72px"><path fill={optionLists.signColors[this.data.color].value} d={optionLists.symbols[this.state.catIdx].items[this.state.symIdx].data}/></svg>
                </td></tr><tr>
                <td></td><td>Color <br/><select onChange={(e)=>this.onChangeColor(e,this.state.line)} value={this.data.color}>{optionLists.signColors.map(itm => (<option key={this.state.line + String(itm.id)} value={itm.id}>{itm.name}</option>))}</select></td>
                <td>Flip Horizontal <br/><input type="checkbox" checked={this.state.flipH} onChange={(e)=>this.onFlipH(e,this.state.line)}/></td>
                <td>Flip Vertical <br/><input type="checkbox" checked={this.state.flipV} onChange={(e)=>this.onFlipV(e,this.state.line)}/></td>
                <td>Editable <br/><input type="checkbox" checked={this.state.canEdit} onChange={(e)=>this.onCanEdit(e,this.state.line)}/></td>
            </tr>
            </React.Fragment>
        );
    }
...
class MultiCtrl extends React.Component {
    ...
    update(data)
    {
        ...
        for(var i in data)
        {
            ...
            if(i < this.lineRefs.length)
                this.lineRefs[i].current.update(data[i]);
        }
    }
	...
	render() {
        ...
        for(var i = 0; i < this.state.maxItems; i++){
            arOptions.push(<option key={"MCI" + String(this.state.signPart) + "_" + String(i)} value={i}> {i} </option>);
            if(i < this.state.curItem)
            {
                ...
                {
                    case 0: // divider lines
                        arRows.push(<DividerRow ref={this.lineRefs[i]} key={key} line={i} callback={this.componentCallback.bind(this)}/>);
                        break;
                    case 1: // text lines
                        arRows.push(<TextRow ref={this.lineRefs[i]} key={key} line={i} callback={this.componentCallback.bind(this)}/>);
                        break;
                    case 2: // shapes
                        arRows.push(<ShapeRow ref={this.lineRefs[i]} key={key} line={i} callback={this.componentCallback.bind(this)}/>);
                        break;
                    case 3: // symbols
                        arRows.push(<SymbolRow ref={this.lineRefs[i]} key={key} line={i} callback={this.componentCallback.bind(this)}/>);
                        break;
                    //case 4: // sizes
            ...
            return (<div style={style}><h3>{this.state.label}&nbsp;<select onChange={this.handleChange.bind(this)} value={this.state.curItem}>{arOptions}</select></h3>
        <table><tbody>{arRows}</tbody></table></div>
        );
	}
}
...
class SizeDivider extends React.Component{
    ...
    render()
    {
        return(<tr><td> &nbsp;&nbsp;&nbsp;Divider {this.state.line+1}</td><td>Position<br/><input style={txtbxStyle} value={this.data.pos} onChange={(e)=>this.posChange(e)}/></td><td>Width<br/><input style={txtbxStyle} value={this.data.w} onChange={(e)=>this.wideChange(e)}/></td></tr>);
    }
}
...
class SizeText extends React.Component{
    ...
    render()
    {
        return(<React.Fragment><tr>
            <td> &nbsp;&nbsp;&nbsp;Line {this.state.line+1}</td>
            <td>Left<br/><input style={txtbxStyle} value={this.data.left} onChange={(e)=>this.leftChange(e)}/></td>
            <td>Top<br/><input style={txtbxStyle} value={this.data.top} onChange={(e)=>this.topChange(e)}/></td>
            <td>Font Size<br/><input style={txtbxStyle} value={this.data.fontSizeOverride} onChange={(e)=>this.fontSizeChange(e)}/></td>
            <td>Font Type<br/><select value={this.data.fontOverride} onChange={(e)=>this.fontTypeChange(e)}>{optionLists.fonts.map(itm => (<option key={this.state.line + String(itm.id)} value={itm.id}>{itm.name}</option>))}</select></td>
            </tr>
            <tr><td></td>
            <td>Spacing<br/><input style={txtbxStyle} value={this.data.spacing} onChange={(e)=>this.spacingChange(e)}/></td>
            <td>Color<br/><select value={this.data.fill} onChange={(e)=>this.colorChange(e)}>{optionLists.signColors.map(itm => (<option key={this.state.line + String(itm.id)} value={itm.id}>{itm.name}</option>))}</select></td>
            </tr></React.Fragment>);
    }
}
var arShapeType = [];
class SizeShape extends React.Component{
    ...
    {
        return(<React.Fragment><tr>
            <td> &nbsp;&nbsp;&nbsp;Shape {this.state.line+1}</td>
            <td>Left<br/><input style={txtbxStyle} value={this.data.x} onChange={(e)=>this.xChange(e)}/></td>
            <td>Top<br/><input style={txtbxStyle} value={this.data.y} onChange={(e)=>this.yChange(e)}/></td>
            <td>{this.getWLabel(arShapeType[this.state.line])}<br/><input style={txtbxStyle} value={this.data.w} onChange={(e)=>this.widthChange(e)}/></td>
            <td>{this.getHLabel(arShapeType[this.state.line])}<br/><input style={txtbxStyle} value={this.data.h} onChange={(e)=>this.heightChange(e)}/></td>
            <td>{this.getRLabel(arShapeType[this.state.line])}<br/><input style={txtbxStyle} value={this.data.r} onChange={(e)=>this.radiusChange(e)}/></td>
            </tr></React.Fragment>);
    }
}
...
class SizeSymbol extends React.Component{
    ...
    render()
    {
        return(<React.Fragment><tr>
            <td> &nbsp;&nbsp;&nbsp;Line {this.state.line+1}</td>
            <td>Height<br/><input style={txtbxStyle} value={this.data.height} onChange={(e)=>this.heightChange(e)}/></td>
            <td>Top<br/><input style={txtbxStyle} value={this.data.top} onChange={(e)=>this.topChange(e)}/></td>
            <td>Center Vertical<br/><input type="checkbox" value={this.data.centerVert} onChange={(e)=>this.centerVertChange(e)}/></td>
            <td>Left<br/><input style={txtbxStyle} value={this.data.left} onChange={(e)=>this.leftChange(e)}/></td>
            <td>Center Horizontal<br/><input type="checkbox" value={this.data.centerHor} onChange={(e)=>this.centerHorChange(e)}/></td>
            </tr></React.Fragment>);
    }
}
...
class VariableElement extends React.Component{
    ...
    render()
    {
        ...
        for(var i = 0; i < this.data.length;i++)
        {
            ...
            {
                case 0:
                    arRows.push(<SizeDivider key={"sdiv_"+i} data={this.data[i]} line={i} callback={this.onChildChanged.bind(this)}/>);
                    break;
                case 1:
                    label = "Lines";
                    arRows.push(<SizeText key={'stext_'+i} line={i} data={this.data[i]} callback={this.onChildChanged.bind(this)}/>);
                    break;
                case 2:
                    label = "Shapes";
                    arRows.push(<SizeShape key={'sshape_'+i} line={i} data={this.data[i]} callback={this.onChildChanged.bind(this)}/>);
                    break;
                case 3:
                    label = "Symbols";
                    arRows.push(<SizeSymbol key={'ssymbol_'+i} line={i} data={this.data[i]} callback={this.onChildChanged.bind(this)}/>);
                    break;
            }
        }
        return(<div><b>{label}</b><table><tbody>{arRows}</tbody></table></div>);
    }
}
...
class SignPreview extends React.Component{
    ...
    render()
    {
        ...
        return (<div style={style} dangerouslySetInnerHTML={{__html:this.preview}}></div>);
    }
}
...
class SignSizes extends React.Component{
    ...
    generateSize(line)
    {
        ...
        if("dividers" in this.data[line] && this.data[line].dividers.length > 0)
        {
            ...
            arSpecs.push(<VariableElement ref={this.childRefs[this.curRef].ref} key={"div_" + line} line={line} type={0} data={this.data[line].dividers} callback={this.varElemCallback.bind(this)}/>);
            this.curRef++;
        }
        if("lines" in this.data[line] && this.data[line].lines.length > 0)
        {
            ...
            this.childRefs[this.curRef].line = line;
            this.childRefs[this.curRef].ref = React.createRef();
            arSpecs.push(<VariableElement ref={this.childRefs[this.curRef].ref} key={"ln_" + line} line={line} type={1} data={this.data[line].lines} callback={this.varElemCallback.bind(this)}/>);
            this.curRef++;
        }
        if("shapes" in this.data[line] && this.data[line].shapes.length > 0)
        {
            ...
            this.childRefs[this.curRef].line = line;
            this.childRefs[this.curRef].ref = React.createRef();
            arSpecs.push(<VariableElement ref={this.childRefs[this.curRef].ref} key={"shp_" + line} line={line} type={2} data={this.data[line].shapes} callback={this.varElemCallback.bind(this)}/>);
            this.curRef++;
        }
        if("sym" in this.data[line] && this.data[line].sym.length > 0)
        {
            ...
            this.childRefs[this.curRef].line = line;
            this.childRefs[this.curRef].ref = React.createRef();
            arSpecs.push(<VariableElement ref={this.childRefs[this.curRef].ref} key={"sym_" + line} line={line} type={3} data={this.data[line].sym} callback={this.varElemCallback.bind(this)}/>);
            this.curRef++;
        }
        this.previewRefs[line] = React.createRef();
        return (<div key={"Size_" + line} style={divStyle}>
            <b>{"Size: " + String(this.data[line].width) + " x " + String(this.data[line].height)}</b>
            <SignPreview ref={this.previewRefs[line]} index={line}/>
            <table>
                <tbody>
                    <tr><td>Width<br/><input onChange={(e)=>this.onChangeWidth(e,line)} style={txtbxStyle} value={this.data[line].width}/></td>
                    <td>Height<br/><input onChange={(e)=>this.onChangeHeight(e,line)} style={txtbxStyle} value={this.data[line].height}/></td>
                    <td>Radius<br/><input onChange={(e)=>this.onChangeRadius(e,line)} style={txtbxStyle} value={this.data[line].rad}/></td>
                    <td>Inner Radius<br/><input onChange={(e)=>this.onChangeInnerRad(e,line)} style={txtbxStyle} value={this.data[line].innerRad}/></td>
                    <td>Inset<br/><input onChange={(e)=>this.onChangeInset(e,line)} style={txtbxStyle} value={this.data[line].inset}/></td></tr>
                    <tr><td>Border Width<br/><input onChange={(e)=>this.onChangeBorderW(e,line)} style={txtbxStyle} value={this.data[line].border}/></td>
                    <td>Line Spacing<br/><input onChange={(e)=>this.onChangeLineSpace(e,line)} style={txtbxStyle} value={this.data[line].lineSpace}/></td>
                    <td>Text Size<br/><input onChange={(e)=>this.onChangeTextSize(e,line)} style={txtbxStyle} value={this.data[line].fontSize}/></td>
                    <td colSpan="2">Default Text Type<br/><select onChange={(e)=>this.onChangeFont(e,line)} value={this.data[line].font}>{optionLists.fonts.map(itm => (<option key={line + String(itm.id)} value={itm.id}>{itm.name}</option>))}</select></td>
                    </tr>
    ...
    render()
    {
        ...
        this.curRef = 0;
        for(var i = 0; i < this.state.maxItems; i++){
            arOptions.push(<option key={"Sizes" + String(this.state.signPart) + "_" + String(i)} value={i}> {i} </option>);
            if(i < this.state.curCount){
                arSizes.push(this.generateSize(i));
            }
        }
        return(<div style={this.divStyle}>
        <h3>{this.label} &nbsp;<select onChange={this.handleChange.bind(this)} value={this.state.curCount}>{arOptions}</select></h3>
        {arSizes}
        </div>);
    }
}
...
function SignAppCombo(props)//type, id, def, callback)
{
    ...
    return(<select id={props.id} value={props.def} onChange={(e)=>props.callback(e,props.id)}>{arItems.map(itm => (<option key={props.id + String(itm.id)} value={itm.id}>{itm.name}</option>))}</select>);
}
...
class SignListCombo extends React.Component{
    ...
    render(){
        return (<select value={this.props.selected} onChange={(e)=>this.action(e.target.value)}>
        {optionLists.signList.map(itm => (<option key={"sign" + String(itm.id)} value={itm.id}>{itm.name}</option>))}
        </select>);
    }
}
...
class AutoSignApp extends React.Component{
    ...
    render(){
        return (
            <div>
                <h1>MUTCD Sign Builder</h1>
                Select: <SignListCombo ref={this.signComboRef} selected={this.autosign.id} action={this.onLoadSign.bind(this)}/>
                <table><tbody><tr><td>
                <table>
                    <tbody>
                    <tr><td>Sign Name</td><td>Sign Shape</td><td>Border Style</td></tr>
                    <tr><td><input placeholder="Sign Name" value={this.autosign.name} onChange={(e) => this.onChangeName(e)}/></td>
                    <td><select value={this.autosign.shape} onChange={(e)=>this.comboChange(e,"signShape")}>{optionLists.signShapes.map(itm => (<option key={"signShape" + String(itm.id)} value={itm.id}>{itm.name}</option>))}</select></td>
                    <td><select value={this.autosign.borderType} onChange={(e)=>this.comboChange(e,"borderStyle")}>{optionLists.borderType.map(itm => (<option key={"borderStyle" + String(itm.id)} value={itm.id}>{itm.name}</option>))}</select></td></tr>
                    <tr><td>Field Color</td><td>Text Color</td><td></td></tr>
                    <tr><td><select value={this.autosign.fieldColor} onChange={(e)=>this.comboChange(e,"fieldColor")}>{optionLists.signColors.map(itm => (<option key={"fieldColor" + String(itm.id)} value={itm.id}>{itm.name}</option>))}</select></td>
                    <td><select value={this.autosign.textColor} onChange={(e)=>this.comboChange(e,"textColor")}>{optionLists.signColors.map(itm => (<option key={"textColor" + String(itm.id)} value={itm.id}>{itm.name}</option>))}</select></td>
                    <td><button onClick={(e)=>this.onSaveSign(e)}>Save</button></td></tr>
                    </tbody>
                </table>
                Set the width and height of all sizes, build the first sign <br/>
                then click <button onClick={(e)=>this.onEstimate(e)}>HERE</button> to auto-calculate the remaining signs.<br/>Only minor tweaking should be needed after that.
                <MultiCtrl ref={this.dividersRef} curItem={this.autosign.dividers.length} maxItems="6" label="Divider Lines" signPart="0" callback={this.multiCallback.bind(this)}/>
                <MultiCtrl ref={this.linesRef} curItem={this.autosign.lines.length} maxItems="11" label="Text Lines" signPart="1" callback={this.multiCallback.bind(this)}/>
                <MultiCtrl ref={this.shapesRef} curItem={this.autosign.shapes.length} maxItems="6" label="Shapes" signPart="2" callback={this.multiCallback.bind(this)}/>
                <MultiCtrl ref={this.symbolsRef} curItem={this.autosign.symbols.length} maxItems="6" label="Symbols" signPart="3" callback={this.multiCallback.bind(this)}/>
                </td><td valign="top">
                <SignSizes ref={this.sizeItemsRef} baseInfo={this.autosign} maxItems="11" label="Sizes" callback={this.multiCallback.bind(this)} />
                </td></tr></tbody></table>
            </div>
        );
    }
